...
int main(int argc,char**argv){
    ...
    if (!interleaved_merge_case(aggregate)) { std::cerr << "v0.24 interleaved merge failed\n"; return EXIT_FAILURE; }
    for(std::size_t i=0U;i<iterations;++i){
        if(!restore_case(rng,aggregate)){std::cerr<<"v0.24 canonical restore failed at "<<i<<'\n';return EXIT_FAILURE;}
        if(!star_case(rng,aggregate)){std::cerr<<"v0.24 star elimination failed at "<<i<<'\n';return EXIT_FAILURE;}
    }
    std::cout<<"v0.24 fuzz iterations="<<iterations<<" aggregate=0x"<<std::hex<<std::uppercase<<aggregate<<std::dec<<'\n';
    return EXIT_SUCCESS;
}

...
def recompute_decision(root: Path) -> tuple[int, str, str, dict[str, Any]]:
    ...
    # P1 legacy/experimental files can be collected and hashed, but version 1.6.0
...
def verify_semantics(root: Path) -> None:
    ...
    if expected["profile"] == "P1":
        if summary.get("ecs_scope_acceptance_status") != "not_implemented_in_1.6.0":
            raise VerificationError("P1 ECS-scope acceptance status is incorrect")
        if status == "passed":
            raise VerificationError("P1 cannot pass in 1.6.0 before complete ECS-scope validators exist")

...
if 'project(NeoEngDCore VERSION 1.13.0 ' not in cmake:
    errors.append('versão CMake divergente: esperado NeoEng D-Core 1.13.0')
...
if profile_registry_path.is_file():
    try:
        ...
        if registry.get('project_version') != '1.13.0':
...
if request_template_path.is_file():
    try:
        ...
        if request_template.get('project_version') != '1.13.0':
...
if gates_path.is_file():
    try:
        gates = json.loads(gates_path.read_text(encoding='utf-8'))
        if gates.get('project_version') != '1.13.0':

...
if 'project(NeoEngDCore VERSION 1.7.0 LANGUAGES C CXX)' not in cmake:
    errors.append('identidade/versão CMake 1.7.0 com C e CXX ausente')
for token in (
...
if header_path.is_file():
    ...
    for token in (
        ...
        'NEOENG_DCORE_RUNTIME_VERSION_MINOR UINT16_C(7)',
...
ledger_path = root / 'audit/CHANGESET_007_CORE_INVARIANT_LEDGER.json'
if ledger_path.is_file():
    try:
        ledger = json.loads(ledger_path.read_text(encoding='utf-8'))
        if ledger.get('schema') != 'neoeng.dcore.changeset-007-core-invariant-ledger.v1':
            errors.append('schema do ledger de invariantes CS007 divergente')
        for row in ledger.get('entries', []):
            path = root / row['path']
            if not path.is_file():
                errors.append(f'arquivo do núcleo ausente no ledger CS007: {row["path"]}')
                continue
            actual = sha(path)
            if row.get('baseline_sha256') != row.get('current_sha256'):
                errors.append(f'ledger declara alteração de núcleo no CS007: {row["path"]}')
            if actual != row.get('current_sha256'):
                errors.append(f'hash atual diverge do ledger de invariantes CS007: {row["path"]}')
        ...
        if current_sources != ledger.get('core_source_list'):
            errors.append('lista de fontes canônicas foi alterada no CS007')
    except Exception as exc:
        errors.append(f'ledger de invariantes CS007 inválido: {exc}')

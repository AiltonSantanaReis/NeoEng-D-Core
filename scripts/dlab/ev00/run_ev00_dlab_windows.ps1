...
function Parse-CtestInventory {
    ...
    if ($summary.Success) {
        ...
        $historicalAllPass = [regex]::Match($text, '100% tests passed out of (\d+)')
        if (-not $historicalAllPass.Success) { throw "Unable to parse CTest summary from $Path" }
        $percent = 100
        $failed = 0
        $total = [int]$historicalAllPass.Groups[1].Value
    }
...
$ResearchBuildDir = Join-Path $RunRoot 'research-build'
...
New-Item -ItemType Directory -Force -Path $RunRoot,$BuildDir,$ResearchBuildDir,$InstallDir,$DepsDir,$EvidenceRoot,(Join-Path $EvidenceRoot 'raw\commands'),(Join-Path $EvidenceRoot 'raw\logs') | Out-Null
...
try {
    ...
    $researchConfigureArgs = @(
        '-S',$SourceDir,'-B',$ResearchBuildDir,'-G','Ninja',
        '-DCMAKE_BUILD_TYPE=Release',
        '-DCMAKE_C_COMPILER=clang-cl',
        '-DCMAKE_CXX_COMPILER=clang-cl',
        "-DCMAKE_TOOLCHAIN_FILE=$toolchain",
    ...
    $researchConfigure = Invoke-RecordedCommand -Name 'research-cmake-configure' -Executable 'cmake' -Arguments $researchConfigureArgs -WorkingDirectory $RunRoot -EvidenceRoot $EvidenceRoot
    if ($researchConfigure.exit_code -ne 0) { throw 'Research replay/history CMake configure failed' }

    $researchBuild = Invoke-RecordedCommand -Name 'research-cmake-build' -Executable 'cmake' -Arguments @('--build',$ResearchBuildDir,'--target','neoeng_dcore_preclosure','neoeng_temporal_closure_tests','--parallel','2') -WorkingDirectory $RunRoot -EvidenceRoot $EvidenceRoot
    if ($researchBuild.exit_code -ne 0) { throw 'Research replay/history target build failed' }

    $replayRegex = '^(neoeng_dcore_replay_smoke|neoeng_dcore_history_smoke|neoeng_temporal_closure_tests)$'
    $replay = Invoke-RecordedCommand -Name 'ctest-replay-rollback' -Executable 'ctest' -Arguments @('--test-dir',$ResearchBuildDir,'-C','Release','--output-on-failure','-R',$replayRegex) -WorkingDirectory $RunRoot -EvidenceRoot $EvidenceRoot
    if ($replay.exit_code -ne 0) { throw 'Replay/rollback research surface failed' }
    $replayInventory = Parse-CtestInventory (Join-Path $EvidenceRoot $replay.stdout)
    $expectedReplayNames = @('neoeng_dcore_history_smoke','neoeng_dcore_replay_smoke','neoeng_temporal_closure_tests') | Sort-Object
    if ($replayInventory.percent -ne 100 -or $replayInventory.failed -ne 0 -or $replayInventory.total -ne 3 -or (($replayInventory.names -join "`n") -ne ($expectedReplayNames -join "`n"))) {
        throw 'Replay/rollback research surface did not execute exactly the three required tests'
    }
    Write-JsonFile ([ordered]@{
        ...
        research_tools = $true
        full_toolset = $false
        expected_tests = $expectedReplayNames
        observed_tests = $replayInventory.names
        total_tests = $replayInventory.total
        failed_tests = $replayInventory.failed
        exact_inventory = $true
    }) (Join-Path $EvidenceRoot 'replay-rollback-validation.json')
    ...
    $historicalPath = Join-Path $ControlRepo 'docs\changesets\015\evidence\github-actions-run-30375982639\raw\windows-clang-cl-ctest.txt'
    if (-not (Test-Path $historicalPath)) { throw "Normative CS015 Windows CTest reference missing: $historicalPath" }
    ...
    Write-JsonFile ([ordered]@{
        ...
        historical_reference = 'docs/changesets/015/evidence/github-actions-run-30375982639/raw/windows-clang-cl-ctest.txt'
    ...
    if (-not $inventoryEqual -or $localCtest.failed -ne 0 -or $historicalCtest.failed -ne 0) {
        throw 'Local CTest inventory/results differ from normative CS015 Windows supported-surface reference'
    }

    $terminalState = 'PASSED'
    $terminalReason = 'supported 54-test surface, isolated research replay/history surface, and all EV-00 qualifying commands passed'
} catch {
    $terminalState = 'FAILED'

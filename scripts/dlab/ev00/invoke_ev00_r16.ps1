...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw 'CS017 R16 wrapper requires PowerShell 7+.' }
if (-not $IsWindows) { throw 'CS017 R16 physical campaign requires Windows.' }
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($planCommit)) { throw 'Unable to derive canonical CS017 R16 plan commit.' }
if ($head -ne $planCommit) { throw "Control HEAD must equal canonical R16 plan commit. HEAD=$head plan=$planCommit" }

$branch = (& git -C $ControlRepo branch --show-current | Out-String).Trim()
if ([string]::IsNullOrWhiteSpace($branch)) { throw 'R16 requires a named local control branch; detached HEAD is rejected.' }
$status = (& git -C $ControlRepo status --porcelain | Out-String).Trim()
if (-not [string]::IsNullOrWhiteSpace($status)) { throw 'Control repository must be clean before R16 Preflight/Qualify.' }
...
function Invoke-ToolchainLinkSmoke {
    foreach ($tool in @('cmake','ninja','clang-cl')) {
        if ($null -eq (Get-Command $tool -ErrorAction SilentlyContinue)) { throw "Required tool not found for R16 compile-link smoke test: $tool" }
    }
    $root = Join-Path ([IO.Path]::GetTempPath()) ("neoeng-ev00-r16-link-smoke-" + [Guid]::NewGuid().ToString('N'))
    $source = Join-Path $root 'src'
    $build = Join-Path $root 'build'
    New-Item -ItemType Directory -Force -Path $source,$build | Out-Null
    try {
        [IO.File]::WriteAllText((Join-Path $source 'CMakeLists.txt'), "cmake_minimum_required(VERSION 3.20)`nproject(neoeng_ev00_r16_link_smoke C)`nadd_executable(neoeng_ev00_r16_link_smoke main.c)`n", [Text.UTF8Encoding]::new($false))
        ...
        if ($LASTEXITCODE -ne 0) { throw "R16 CMake compile-link smoke configure failed:`n$configure" }
        $buildOutput = (& cmake --build $build --parallel 1 2>&1 | Out-String)
        if ($LASTEXITCODE -ne 0) { throw "R16 CMake compile-link smoke build failed:`n$buildOutput" }
        $exe = Get-ChildItem -Path $build -Filter 'neoeng_ev00_r16_link_smoke.exe' -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($null -eq $exe) { throw 'R16 compile-link smoke did not produce the expected executable.' }
    } finally {
...
function Import-CtestParser {
    ...
    if ($parseErrors.Count -ne 0) { throw 'R16 cannot validate CTest parser because canonical runner has syntax errors.' }
    ...
    if ($null -eq $fn) { throw 'R16 canonical runner does not define Parse-CtestInventory.' }
...
function Assert-CtestParserCompatibility {
    ...
    if (-not (Test-Path -LiteralPath $normative)) { throw "R16 normative CS015 Windows CTest reference missing: $normative" }
    ...
    if ($parsed.percent -ne 100 -or $parsed.failed -ne 0 -or $parsed.total -ne 54 -or @($parsed.names).Count -ne 54) {
        throw 'R16 canonical parser did not resolve normative CS015 inventory as 100%/0/54/54-names.'
    }
}
...
function Assert-DualSurfaceContract {
    ...
    foreach ($name in @('neoeng_dcore_replay_smoke','neoeng_dcore_history_smoke')) {
        if (@($supported.names) -contains $name) { throw "R16 normative 54-test surface unexpectedly contains research-only test: $name" }
    }

    $cmakeText = (& git -C $ControlRepo show "${ProductSha}:CMakeLists.txt" | Out-String)
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($cmakeText)) { throw 'Unable to read protected baseline CMakeLists.txt for R16 contract audit.' }
    foreach ($token in @('if(NEOENG_DCORE_BUILD_RESEARCH_TOOLS)','neoeng_dcore_add_tool(neoeng_dcore_preclosure apps/v28_year1_preclosure.cpp)','add_test(NAME neoeng_dcore_replay_smoke','add_test(NAME neoeng_dcore_history_smoke','neoeng_temporal_closure_tests')) {
        if (-not $cmakeText.Contains($token)) { throw "Protected baseline lacks required R16 research contract token: $token" }
    }
    ...
    foreach ($token in @('$ResearchBuildDir',"'-DNEOENG_DCORE_BUILD_RESEARCH_TOOLS=OFF'","'-DNEOENG_DCORE_BUILD_RESEARCH_TOOLS=ON'","-Name 'research-cmake-configure'","-Name 'research-cmake-build'","'neoeng_dcore_preclosure'","'neoeng_temporal_closure_tests'","'^(neoeng_dcore_replay_smoke|neoeng_dcore_history_smoke|neoeng_temporal_closure_tests)$'",'replay-rollback-validation.json')) {
        if (-not $runnerText.Contains($token)) { throw "R16 canonical runner lacks dual-surface contract token: $token" }
    }

    $snapshotText = [IO.File]::ReadAllText($VerifierSnapshot)
    foreach ($token in @('EXPECTED_RESEARCH_TESTS','research-cmake-configure','research-cmake-build','supported-surface configure is not research-OFF','replay/history configure is not research-ON','supported-surface CTest inventory differs from normative CS015 54-test inventory','replay/history surface must be exactly 3/0 and exact names')) {
        if (-not $snapshotText.Contains($token)) { throw "Preserved R15 verifier lacks dual-surface runtime token: $token" }
    }
    ...
    foreach ($token in @('verify_ev00_dlab_evidence_r15.py','research_contract_self_test_fixed','started_at_utc','finished_at_utc','r15.check_replay_rollback')) {
        if (-not $currentText.Contains($token)) { throw "R16 verifier fixture correction token missing: $token" }
    }
}
...
function Invoke-VerifierSelfTest {
    ...
    if ($LASTEXITCODE -ne 0) { throw "R16 independent verifier self-test failed:`n$output" }
    if (-not $output.Contains('EV00 EVIDENCE VERIFIER SELF-TEST: PASS')) { throw "R16 verifier self-test did not emit PASS marker:`n$output" }
}
...
$verifierSnapshot = Join-Path $PSScriptRoot 'verify_ev00_dlab_evidence_r15.py'
foreach ($p in @($runner,$verifier,$verifierSnapshot)) { if (-not (Test-Path -LiteralPath $p)) { throw "R16 required EV-00 artifact missing: $p" } }

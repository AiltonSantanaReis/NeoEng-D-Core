...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw 'CS017 R19 wrapper requires PowerShell 7+.' }
if (-not $IsWindows) { throw 'CS017 R19 physical campaign requires Windows.' }
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($planCommit)) { throw 'Unable to derive canonical CS017 R19 plan commit.' }
if ($head -ne $planCommit) { throw "Control HEAD must equal canonical R19 plan commit. HEAD=$head plan=$planCommit" }

$branch = (& git -C $ControlRepo branch --show-current | Out-String).Trim()
if ([string]::IsNullOrWhiteSpace($branch)) { throw 'R19 requires a named local control branch; detached HEAD is rejected.' }
...
if (-not [string]::IsNullOrWhiteSpace($statusBefore)) { throw "Control repository must be clean before R19 wrapper checks:`n$statusBefore" }
...
if (-not [string]::IsNullOrWhiteSpace($statusPreDelegate)) { throw "R19 wrapper-side setup dirtied the control tree before delegation:`n$statusPreDelegate" }
...
if (-not [string]::IsNullOrWhiteSpace($statusAfter)) {
    throw "R19 wrapper/runner delegation changed the control tree; refusing result:`n$statusAfter"
}

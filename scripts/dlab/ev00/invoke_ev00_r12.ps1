...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw 'CS017 R12 wrapper requires PowerShell 7+.' }
if (-not $IsWindows) { throw 'CS017 R12 physical campaign requires Windows.' }
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($planCommit)) { throw 'Unable to derive canonical CS017 R12 plan commit.' }
if ($head -ne $planCommit) { throw "Control HEAD must equal canonical R12 plan commit. HEAD=$head plan=$planCommit" }

$branch = (& git -C $ControlRepo branch --show-current | Out-String).Trim()
if ([string]::IsNullOrWhiteSpace($branch)) { throw 'R12 requires a named local control branch; detached HEAD is rejected.' }
$status = (& git -C $ControlRepo status --porcelain | Out-String).Trim()
if (-not [string]::IsNullOrWhiteSpace($status)) { throw 'Control repository must be clean before R12 Preflight/Qualify.' }
...
function Invoke-ToolchainLinkSmoke {
    foreach ($tool in @('cmake','ninja','clang-cl')) {
        if ($null -eq (Get-Command $tool -ErrorAction SilentlyContinue)) { throw "Required tool not found for R12 compile-link smoke test: $tool" }
    }

    $root = Join-Path ([IO.Path]::GetTempPath()) ("neoeng-ev00-r12-link-smoke-" + [Guid]::NewGuid().ToString('N'))
    $source = Join-Path $root 'src'
    $build = Join-Path $root 'build'
    New-Item -ItemType Directory -Force -Path $source,$build | Out-Null
    try {
        [IO.File]::WriteAllText((Join-Path $source 'CMakeLists.txt'), "cmake_minimum_required(VERSION 3.20)`nproject(neoeng_ev00_r12_link_smoke C)`nadd_executable(neoeng_ev00_r12_link_smoke main.c)`n", [Text.UTF8Encoding]::new($false))
        ...
        if ($LASTEXITCODE -ne 0) { throw "R12 CMake compile-link smoke configure failed:`n$configure" }
        $buildOutput = (& cmake --build $build --parallel 1 2>&1 | Out-String)
        if ($LASTEXITCODE -ne 0) { throw "R12 CMake compile-link smoke build failed:`n$buildOutput" }

        $exe = Get-ChildItem -Path $build -Filter 'neoeng_ev00_r12_link_smoke.exe' -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($null -eq $exe) { throw 'R12 compile-link smoke did not produce the expected executable.' }
    } finally {
...
if (-not (Test-Path -LiteralPath $normative)) { throw "R12 normative CS015 Windows CTest reference missing: $normative" }

$runner = Join-Path $PSScriptRoot 'run_ev00_dlab_windows.ps1'
if (-not (Test-Path -LiteralPath $runner)) { throw "R12 EV-00 runner missing: $runner" }

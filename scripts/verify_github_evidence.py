...
def api_json(url: str, token: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict): raise ValueError(f"GitHub API response is not object: {url}")
    return value
...
def verify_run(repo: str, token: str, run_id: int, expected_sha: str, label: str) -> list[str]:
    ...
    if run.get("id") != run_id: errors.append(f"{label}: run id mismatch")
    if run.get("head_sha") != expected_sha: errors.append(f"{label}: head_sha mismatch")
    if run.get("status") != "completed": errors.append(f"{label}: run not completed")
    if run.get("conclusion") != "success": errors.append(f"{label}: conclusion is not success")
    ...
    if isinstance(repository, dict) and repository.get("full_name") != repo:
        errors.append(f"{label}: repository identity mismatch")
    return errors
...
def verify_pr(repo: str, token: str, pr_number: int, expected_head: str, expected_merge: str) -> list[str]:
    ...
    head = pr.get("head")
    if not isinstance(head, dict) or head.get("sha") != expected_head: errors.append(f"PR #{pr_number}: head SHA mismatch")
    if pr.get("merged") is not True: errors.append(f"PR #{pr_number}: not merged")
    if pr.get("merge_commit_sha") != expected_merge: errors.append(f"PR #{pr_number}: merge SHA mismatch")
    if pr.get("base", {}).get("ref") != "main": errors.append(f"PR #{pr_number}: base is not main")
    return errors
...
def verify_chain(repo: str, token: str, root: Path) -> list[str]:
    ...
    if not isinstance(entries, list): return ["acceptance chain entries missing"]
...
def main() -> int:
    ...
    args = ap.parse_args()
    if not args.repo: print("GITHUB EVIDENCE VERIFICATION: REJECT\n- repository identity missing"); return 1
    try:
        if args.offline_schema_only:
            ...
            if not token: errors = [f"required token environment variable missing: {args.token_env}"]
            else: errors = verify_chain(args.repo, token, Path(args.root))

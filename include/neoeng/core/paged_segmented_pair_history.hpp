...
struct PagedSegmentedPairHistoryConfig final {
    std::size_t bodies{};
    std::size_t maximum_contacts{};
    std::size_t maximum_pairs{};
    std::size_t maximum_pairs_per_segment{};
    std::size_t history_capacity{32U};
    std::size_t table_page_elements{256U};
    std::size_t segment_generations{};
    std::size_t spill_generations{};
    std::size_t table_generations{};
    std::size_t body_key_page_generations{};
    std::size_t segment_map_page_generations{};
    FixedSlotAllocatorMode allocation_mode{FixedSlotAllocatorMode::NextFit};
};

struct PagedSegmentedPairHistoryStats final {
    std::uint64_t snapshots_captured{};
    std::uint64_t tables_shared{};
    std::uint64_t tables_written{};
    std::uint64_t body_pages_shared{};
    std::uint64_t body_pages_written{};
    std::uint64_t segment_pages_shared{};
    ...
    std::uint64_t direct_ordered_restores{};
    std::uint64_t merged_restores{};
    std::uint64_t merge_heap_pushes{};
    std::size_t live_pairs{};
    std::size_t live_islands{};
    std::size_t live_spill_pairs{};
};

// v0.23: fixed-capacity pair history with two COW layers.
//
// 1. Each contact island owns an immutable pair segment.
// 2. The island-key and island->segment tables are split into immutable pages.
//
// A frame with one dirty island writes at most one segment-map page (plus a new
// segment payload when the pair set changed). Stable frames share the whole table.
class PagedSegmentedPairHistory final {
public:
    explicit PagedSegmentedPairHistory(PagedSegmentedPairHistoryConfig config);
    ...
    [[nodiscard]] const PagedSegmentedPairHistoryStats& stats() const noexcept { return stats_; }
...
private:
    ...
    struct BodyPageGeneration final { std::uint32_t refs{}; };
    struct SegmentPageGeneration final { std::uint32_t refs{}; };
    ...
    struct MergeCursor final {
        const BroadphasePair* data{};
        std::uint32_t count{};
        std::uint32_t index{};

        [[nodiscard]] const BroadphasePair& value() const noexcept { return data[index]; }
    };
    ...
    [[nodiscard]] std::uint32_t acquire_body_page();
    [[nodiscard]] std::uint32_t acquire_segment_page();
    ...
    void release_body_page(std::uint32_t page) noexcept;
    void release_segment_page(std::uint32_t page) noexcept;

    [[nodiscard]] std::uint32_t* table_body_pages(std::uint32_t table) noexcept;
    [[nodiscard]] const std::uint32_t* table_body_pages(std::uint32_t table) const noexcept;
    [[nodiscard]] std::uint32_t* table_segment_pages(std::uint32_t table) noexcept;
    [[nodiscard]] const std::uint32_t* table_segment_pages(std::uint32_t table) const noexcept;
    [[nodiscard]] std::size_t* body_page_storage(std::uint32_t generation) noexcept;
    [[nodiscard]] const std::size_t* body_page_storage(std::uint32_t generation) const noexcept;
    [[nodiscard]] std::uint32_t* segment_page_storage(std::uint32_t generation) noexcept;
    [[nodiscard]] const std::uint32_t* segment_page_storage(std::uint32_t generation) const noexcept;
    ...
    [[nodiscard]] bool same_segment(std::uint32_t generation, std::span<const KeyedPair> run) const noexcept;
    [[nodiscard]] bool same_spill(std::uint32_t generation, std::span<const BroadphasePair> pairs) const noexcept;
    [[nodiscard]] bool body_page_equals(std::uint32_t generation, std::size_t page) const noexcept;
    [[nodiscard]] bool segment_page_equals(std::uint32_t generation, std::size_t page) const noexcept;
    [[nodiscard]] std::uint32_t body_page_at(const TableGeneration& table, std::uint32_t table_index,
                                             std::size_t page) const noexcept;
    [[nodiscard]] std::uint32_t segment_page_at(const TableGeneration& table, std::uint32_t table_index,
                                                std::size_t page) const noexcept;

    PagedSegmentedPairHistoryConfig config_{};
    ...
    std::size_t body_page_generation_count_{};
    std::size_t segment_page_generation_count_{};

    std::vector<SegmentGeneration> segment_generations_{};
    std::vector<SpillGeneration> spill_generations_{};
    std::vector<BodyPageGeneration> body_page_generations_{};
    std::vector<SegmentPageGeneration> segment_page_generations_{};
    std::vector<TableGeneration> tables_{};
    std::vector<BroadphasePair> segment_payload_{};
    std::vector<BroadphasePair> spill_payload_{};
    std::vector<std::size_t> body_page_payload_{};
    std::vector<std::uint32_t> segment_page_payload_{};
    std::vector<std::uint32_t> table_body_page_ids_{};
    std::vector<std::uint32_t> table_segment_page_ids_{};
    std::vector<Snapshot> snapshots_{};

    std::vector<std::size_t> parent_{};
    std::vector<std::size_t> current_body_keys_{};
    std::vector<std::uint32_t> current_segments_{};
    ...
    std::vector<std::uint32_t> staged_body_pages_{};
    std::vector<std::uint32_t> staged_segment_pages_{};
    mutable std::vector<MergeCursor> restore_heap_{};

    std::size_t staged_segment_count_{};
    std::size_t staged_body_page_count_{};
    std::size_t staged_segment_page_count_{};
    ...
    mutable PagedSegmentedPairHistoryStats stats_{};
    FixedSlotAllocator table_allocator_{};
    FixedSlotAllocator segment_allocator_{};
    FixedSlotAllocator spill_allocator_{};
    FixedSlotAllocator body_page_allocator_{};
    FixedSlotAllocator segment_page_allocator_{};
    bool initialized_{};
};
